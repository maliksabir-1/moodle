...
// Certificates issued via tool_certificate (e.g. mod_coursecertificate, local_point_badges) are not stored in customcert.
if (
    !$DB->count_records('customcert_issues', ['userid' => $userid])
    && file_exists($CFG->dirroot . '/admin/tool/certificate/lib.php')
    && $DB->count_records('tool_certificate_issues', ['userid' => $userid])
) {
    $redirectparams = [];
    if ($userid != $USER->id) {
        $redirectparams['userid'] = $userid;
    }
    redirect(new moodle_url('/admin/tool/certificate/my.php', $redirectparams));
}
