...
class custom_completion extends activity_custom_completion {
    ...
    public function get_state(string $rule): int {
        ...
        if ($rule === 'completionrequiretime') {
            ...
            $track = $DB->get_record('timedactivity_tracking', ['timedactivityid' => $this->cm->instance, 'userid' => $this->userid]);
        ...
        if ($rule === 'completionpass') {
            require_once($this->config->dirroot . '/mod/timedactivity/lib.php');
            $grade = timedactivity_get_user_grade($timedactivity, $this->userid);
            if ($grade === null) return COMPLETION_INCOMPLETE;
    ...
    public function get_custom_rule_descriptions(): array {
        ...
        return [
            'completionrequiretime' => get_string('completionrequiretime_desc', 'mod_timedactivity', format_time($timedactivity->requiredtime)),
            'completionpass' => get_string('completionpass_desc', 'mod_timedactivity', $timedactivity->passinggrade),
            'completionallquizzes' => get_string('completionallquizzes_desc', 'mod_timedactivity'),
        ];
    }
