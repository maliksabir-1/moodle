...
namespace theme_mycleantheme\output\core;
...
class course_renderer extends \core_course_renderer {
     ...
     * Overridden to display modern cards with progress bars and category tree.
     *
    ...
    public function course_category($category) {
        ...
        $categorytree = $this->build_category_tree($coursecat);
        
        // Get all courses data keyed by category ID.
        $coursesbycategory = $this->get_all_courses_by_category($coursecat);

        // Convert courses data to JSON for JavaScript.
        $coursesbycategoryjson = json_encode($coursesbycategory, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP);
        ...
        $context = [
            ...
            'coursesbycategoryjson' => $coursesbycategoryjson,
            'hascategories' => !empty($categorytree),
            'hascourses' => !empty($coursesbycategory)
        ];

        return $this->render_from_template('theme_mycleantheme/all_courses', $context);
    }

    /**
     * Build recursive category tree structure.
     *
            ...
            if ($child->is_uservisible()) {
                ...
                if (!empty($grandchildren)) {
                    ...
                    foreach ($grandchildren as $grandchild) {
                        if ($grandchild->is_uservisible()) {
                            $node['children'][] = [
                                'id' => $grandchild->id,
                                'name' => format_string($grandchild->name),
                                'coursecount' => $grandchild->get_courses_count(['recursive' => true]),
                                'parentid' => $child->id
                            ];
                        }
                    }
     ...
     * @return array
     */
    private function get_all_courses_by_category($category) {
        global $CFG, $DB, $USER;
        ...
        if ($category->id) {
            $courseslist = $category->get_courses(['recursive' => true]);
        } else {
        ...
        foreach ($courseslist as $c) {
            ...
            if ($c->visible || has_capability('moodle/course:viewhiddencourses', $coursecontext)) {
                ...
                // Get last access
                $lastaccess = $DB->get_field('user_lastaccess', 'timeaccess', ['userid' => $USER->id, 'courseid' => $c->id]);
                if (!$lastaccess) {
                    $lastaccess = 0;
                }
                
                // Determine course status (inprogress, future, past)
                $time = time();
                $status = 'inprogress';
                ...
                if (!isset($coursesbycategory[$c->category])) {
                    $coursesbycategory[$c->category] = [];
                }
                $coursesbycategory[$c->category][] = $course;
            }
        }
